 ...
 * @author BLIS
 */
public class MindrayBS300 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("Mindray BS-300 handler stopped", DisplayMessageType.TITLE);
      ...
      @Override
    public void run() {
        log.AddToDisplay.Display("Mindray BS-300 handler started...", DisplayMessageType.TITLE);
        ...
        try
        {
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
                ...
                log.AddToDisplay.Display("Mindray BS-300 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                ClientThread client = new ClientThread(connSock,"Mindray BS-300");
                client.start();
                String message ;
                  ...
                  //  String patientid = pidParts[1];
                   ...
                   // SampleID =  patientid;
                    ...
                    for(int i=3;i<msgParts.length;i++)
                    {
                        ...
                        {
                            mID = getMeasureID(msgParts[i].split("\\|")[3]);
                            if(mID > 0)
                            {
                                try
                                {
