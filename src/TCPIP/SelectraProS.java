 ...
 * @author BLIS
 */
public class SelectraProS extends Thread{
    ...
    private static List<String> calctestData = new ArrayList<String>();
    ...
    public void Stop()
    {
        try {
            ...
            if(null !=welcomeSocket)
                 welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("SelectraProS handler stopped", DisplayMessageType.TITLE);
    ...
    public void run() {             
        ...
        try
        {
            log.AddToDisplay.Display("SelectraProS handler started...", DisplayMessageType.TITLE); 
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
            ...
            else
            {
               ...
               connSock = new Socket(tcpsettings.EQUIPMENT_IP, tcpsettings.PORT);
                ...
                log.AddToDisplay.Display("SelectraProS is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                if(!tcpsettings.SERVER_MODE)
                {
                    connSock.setKeepAlive(true);
                }
                ClientThread client = new ClientThread(connSock,"SelectraProS");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("SelectraProS client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
            ...
            {            
                ...
                { 
                    appMode = MODE.SENDING_QUERY;                    
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to SelectraProS",DisplayMessageType.INFORMATION);
   ...
   private static boolean sendNow(String message)
   {
       boolean status = false;
        try {
            ...
            Logger.getLogger(SelectraProS.class.getName()).log(Level.SEVERE, null, ex);
        }
       log.AddToDisplay.Display("[ "+message+ " ] sent successfully",DisplayMessageType.INFORMATION);
   ...
   private static void noDataResponse()
   {
       ...
       strTemp.append("||BLIS|||||SelectraProS PC^SelectraProS^4.3||P|1394-97|");       
   ...
   private static void prepare(sampledata get)
   {
      ...
      {       
           ...
           if(!ASTMMsgs.isEmpty())
           {
                   ...
                   String[] PatientParts = ASTMMsgs.trim().split(String.valueOf(STX));
                    for(int p=0;p<PatientParts.length;p++)
                    {
                        ...
                        if(pidParts.length > 4)
                        {
                            if("Q".equals(pidParts[0].trim()))
                            {
                                ...
                                */
                                ...
                                //log.AddToDisplay.Display("Request from Analyzer to download sample details",DisplayMessageType.INFORMATION);
                            }
                            else if("C".equals(pidParts[0].trim()))
                            {
                                log.AddToDisplay.Display("last request has been cancelled by SelectraProS",DisplayMessageType.INFORMATION);
                                 appMode = MODE.IDLE;
                            ...
                            else if("O".equals(pidParts[0].trim()))
                            {
                                pidParts = msgParts[2].split("\\|");
                                patientid = pidParts[2].split("\\^")[0].trim();
                                SampleID = pidParts[2].split("\\^")[0].trim(); 
                                //SampleID = utilities.getSystemDate("YYYY") + SampleID;
                                //SampleID =  patientid;
                                if(SampleID.length() == 0)
                                {
                                    log.AddToDisplay.Display("Empty Specimen ID format not known: Skipped",DisplayMessageType.INFORMATION);
                                    continue;
                                }
                                ...
                                for(int i=0;i<msgParts.length;i++)
                                {
                                    ...
                                    {
                                        eqmID = msgParts[i].split("\\|")[2].split("\\^")[3].trim();
                                        mID = getMeasureID(eqmID);
                                        if(mID > 0)
                                        {
                                            try
                                            {
                                                value = msgParts[i].split("\\|")[3].split("\\^")[0].trim();
                                            }catch(ArrayIndexOutOfBoundsException ex){ continue;}
                                            
                                            
                                            PrepareCalcTests(eqmID,value);
                        ...
                        for(int i=0;i<calctestData.size();i++)
                        {
                            mID = getMeasureID(calctestData.get(i).split(";")[0].trim());
                            if(mID > 0)
                            {
                                try
                                {
                                    ...
                                    cvalue = Float.parseFloat(String.valueOf(engine.eval(calctestData.get(i).split(";")[1])));
                                }catch(NumberFormatException e){
                                    continue;
                                } catch (ScriptException ex) {
                                    log.logger.Logger("Could not perform calculation:"+calctestData.get(i).split(";")[1]);
                                    //log.logger.PrintStackTrace(ex);
                                    continue;
                                }                                        

                                if(SaveResults(SampleID, mID,String.valueOf(cvalue)))
                                {
                                    flag = true;
                                }
                            }                        
                        ...
                        if(flag)
                        {
                             log.AddToDisplay.Display("\nResult for Test with Specimen ID: "+SampleID +" sent to BLIS sucessfully",DisplayMessageType.INFORMATION);
                        }
                        else
                        {
                             log.AddToDisplay.Display("\nTest with Specimen ID: "+SampleID +" not Found on BLIS",DisplayMessageType.WARNING);
                        }
    ...
    private static MSGTYPE getMessageType(String msg)
    {
     ...
     private static void PrepareCalcTests(String equipmentID, String value)
     {
        // int measureid = 0;
         ...
         for(int i=0;i<calctestData.size();i++)
         {
             String temp2=calctestData.get(i).replaceAll(equipmentID, value); 
             temp.add(temp2);                        
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/selectraProS/selectraProS.xml");
        try {
