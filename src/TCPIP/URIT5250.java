 ...
 * @author BLIS
 */
public class URIT5250 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            if(null !=welcomeSocket)
                 welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("URIT 5250 handler stopped", DisplayMessageType.TITLE);
    ...
    public void run() {             
        ...
        try
        {
            log.AddToDisplay.Display("URIT 5250 handler started...", DisplayMessageType.TITLE); 
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
            ...
            else
            {
               ...
               connSock = new Socket(tcpsettings.EQUIPMENT_IP, tcpsettings.PORT);
                ...
                log.AddToDisplay.Display("URIT 5250 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                if(!tcpsettings.SERVER_MODE)
                {
                    connSock.setKeepAlive(true);
                }
                ClientThread client = new ClientThread(connSock,"URIT 5250");
                client.start();
                String message ;
		...
		{
                    log.AddToDisplay.Display("URIT 5250 client is now disconnected!",DisplayMessageType.WARNING);
                    log.logger.Logger(e.getMessage());
		}
            ...
            {            
                ...
                { 
                    appMode = MODE.SENDING_QUERY;                    
                       log.AddToDisplay.Display("Sending test with Code: "+SampleList.get(i).aux_id + " to URIT 5250",DisplayMessageType.INFORMATION);
   ...
   private static boolean sendNow(String message)
   {
       boolean status = false;
        try {
            ...
            Logger.getLogger(URIT5250.class.getName()).log(Level.SEVERE, null, ex);
        }
       log.AddToDisplay.Display("[ "+message+ " ] sent successfully",DisplayMessageType.INFORMATION);
   ...
   private static void noDataResponse()
   {
       ...
       strTemp.append("||BLIS|||||URIT 5250 PC^URIT 5250^4.3||P|1394-97|");       
   ...
   private static void prepare(sampledata get)
   {
                  ...
                  ASTMMsgs = ASTMMsgs.replaceAll(String.valueOf(ETX), "");
                   ...
                   String[] PatientParts = ASTMMsgs.trim().split("L\\|1\\|N"+String.valueOf(CR));
                    for(int p=0;p<PatientParts.length;p++)
                    {
                        ...
                        String pidParts[] = msgParts[2].split("\\|");
                        if(pidParts.length > 4)
                        {
                                pidParts = msgParts[2].split("\\|");
                                String patientid = pidParts[4].trim();
                                String SampleID = patientid;
                                
                                if(patientid.isEmpty())
                                {
                                    log.AddToDisplay.Display("\nInvalid Specimen ID ["+SampleID + "] result rejected",DisplayMessageType.WARNING);
                                    continue;
                                }
                                ...
                                for(int i=0;i<msgParts.length;i++)
                                {
                                    ...
                                    {
                                        mID = getMeasureID(msgParts[i].split("\\|")[2].split("\\^")[1].trim());
                                        if(mID > 0)
                                        {
                                            try
                                            {
    ...
    {
        String data = "";
        xmlparser p = new xmlparser("configs/URIT/urit5250.xml");
        try {
    ...
    private static boolean SaveResults(String barcode,int MeasureID, String value)
     {
          ...
          boolean flag = false; 
          String conblis = BLIS.blis.saveResults(barcode,MeasureID,value);
          if("1".equals(conblis))
           {
              flag = true;
            }
          ...
          {
              log.AddToDisplay.Display("Could not save data into BLIS["+value+"] Error code:["+conblis+"]",DisplayMessageType.ERROR);
          }
