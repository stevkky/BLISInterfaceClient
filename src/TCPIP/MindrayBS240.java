 ...
 * @author BLIS
 */
public class MindrayBS240 extends Thread{
    ...
    public void Stop()
    {
        try {
            ...
            welcomeSocket.close();
//            connSock.close();
             log.AddToDisplay.Display("Mindray BS-240 handler stopped", DisplayMessageType.TITLE);
      ...
      @Override
    public void run() {
        log.AddToDisplay.Display("Mindray BS-240 handler started...", DisplayMessageType.TITLE);     
  		...
  		log.AddToDisplay.Display("Listening on port "+ tcpsettings.PORT+"...",DisplayMessageType.INFORMATION);
            ...
            else
            {
               ...
               connSock = new Socket(tcpsettings.EQUIPMENT_IP, tcpsettings.PORT);
                ...
                log.AddToDisplay.Display("Mindray BS-240 is now Connected...",DisplayMessageType.INFORMATION);
                first=false;
                ClientThread client = new ClientThread(connSock,"Mindray BS-240");
                client.start();
                String message ;
    ...
    {        
        ...
        try {
            ...
            else if(MessageType.OBSERVE_RESULT.toString().equalsIgnoreCase(msg.type))
            {
                ...
                {
                    String patientid = pidParts[2];
                    String SampleID = patientid;
                   
                    if(patientid.isEmpty())
                    {
                        SampleID = msgParts[2].split("\\|")[2].trim();
                    }
                    ...
                    if(msgParts.length > 2)
                    {
                        for(int i=3;i<msgParts.length;i++)
                        {
                            if(msgParts[i].startsWith("OBX"))
                            {
                                  try
                                  {
                                    mID = Integer.parseInt(msgParts[i].split("\\|")[3]);
                                ...
                                if(mID > 0)
                                {
                                    try
                                    {
                                        value = Float.parseFloat(msgParts[i].split("\\|")[13]);
